//-------------------------------------------------------------------------------------------------//
/*! \file
 *  \brief      Implementation of ABBAUWApplBase (application controller).
 *              Revised: creates PurchaseDBParameterProxy from command-line args
 *              (m_DB -> PURCHASE_DB, m_Server -> PURCHASE_DBSRV) and passes it
 *              into PuBatchOrderPosDM via PuBatchOrderPosInitParams so that
 *              PURCHASE_DB / PURCHASE_DBSRV resolve correctly in the remote
 *              dblink SQL.  Mirrors the tender pattern without requiring a
 *              Basar parameter-gateway header.
 */
//-------------------------------------------------------------------------------------------------//
...
// Purchase DB proxy (concrete type needed here for construction + gateway injection)
...
namespace basar
{
	...
	{
template<>
typename Singleton<abbaUW::ABBAUWApplBase>::InstancePtr&
Singleton<abbaUW::ABBAUWApplBase>::inst()
		{
...
//-------------------------------------------------------------------------------------------------//
    ...
    : m_Logger( LoggerPool::LoggerAbbaUW )
    , m_InitFlag( "ABBAUWApplBase::init" )
    {
...
//-------------------------------------------------------------------------------------------------//
	...
	void ABBAUWApplBase::checkCountryCodeValidity()		
	{
        ...
        str << "Countrycode-Parameter=" << i18nstr
            << ". It must be CH, DE1, DE2 or AT." << std::endl;
        throw basar::cmnutil::CmdLineException(
            basar::ExceptInfo( fun, str.str().c_str(), __FILE__, __LINE__ ) );
		}
    ...
    // Connection from UserAdmin for orderproposal (outbound)
    const libBatch::dbConnection::DBConnectionConfiguration& dbConfig =
        appInitializer.getDBConnectionConfiguration();
		databaseInfoOrderprop.setInfxConcurrent(dbConfig.m_DBSrv.c_str(), dbConfig.m_DB.c_str());
		
    // Connection from command-line args for pubatchorderpos (inbound)
    ...
    m_AppSession = AppSessionPtr(
        new AppSession(
            libBatch::dbConnection::DBConnectionBasePtr(
                new dbConnection::DBIfxConnection( databaseInfoOrderprop ) ),
            libBatch::dbConnection::DBConnectionBasePtr(
                new dbConnection::DBIfxConnection( databaseInfoPuBatch ) ),
    ...
    // -----------------------------------------------------------------------
    // Create PurchaseDBParameterProxy (mirrors tender pattern).
    //
    // The dblink identifiers PURCHASE_DB and PURCHASE_DBSRV used inside the
    // pubatchorderpos SQL come directly from the command-line args that also
    // establish the inbound DB connection: -db <name> and -server <srv>.
    //
    // m_DB   -> PURCHASE_DB    (Informix dblink database name)
    // m_Server -> PURCHASE_DBSRV (Informix dblink server name)
    //
    // No Basar parameter-gateway header is required; the proxy simply stores
    // and returns these two plain strings.
    // -----------------------------------------------------------------------
    m_PurchaseDBParameterProxy =
        abbaUW::infrastructure::db::PurchaseDBParameterProxyPtr(
            new abbaUW::infrastructure::db::PurchaseDBParameterProxy( db, server ) );
    }
...
boost::shared_ptr<domMod::OrderProposalTransfer::OrderProposalDM>
ABBAUWApplBase::getOrderProposalDM()
    {
        if( NULL == m_OrderProposalDM.get() )
		{
        m_OrderProposalDM = boost::shared_ptr<domMod::OrderProposalTransfer::OrderProposalDM>(
            new domMod::OrderProposalTransfer::OrderProposalDM(
                m_AppSession->m_DBConnectionOutbound->getCurrentConnection() ) );
...
// ---------------------------------------------------------------------------
// KEY CHANGE: getPuBatchOrderPosDM() now passes the PurchaseDBParameterProxy
// through PuBatchOrderPosInitParams so the DM can stamp PURCHASE_DB /
// PURCHASE_DBSRV on every iterator before calling execute().
// This mirrors tender's PurchaseOrderProposalDM / PurchaseDBParameterProxy
// wiring in libtenderapplcompmgr.
// ---------------------------------------------------------------------------
boost::shared_ptr<domMod::OrderProposalTransfer::PuBatchOrderPosDM>
ABBAUWApplBase::getPuBatchOrderPosDM()
    {
        if( NULL == m_PuBatchOrderPosDM.get() )
		{
        m_PuBatchOrderPosDM = boost::shared_ptr<domMod::OrderProposalTransfer::PuBatchOrderPosDM>(
            new domMod::OrderProposalTransfer::PuBatchOrderPosDM(
                m_AppSession->m_DBConnectionInbound->getCurrentConnection() ) );

			domMod::OrderProposalTransfer::PuBatchOrderPosInitParams param;

        // Pass the proxy so doInit() stores it; the DM will call
        // proxy->getPurchaseDB() / getPurchaseDBServer() before every execute().
        param.purchaseDBParameterProxy = m_PurchaseDBParameterProxy;
