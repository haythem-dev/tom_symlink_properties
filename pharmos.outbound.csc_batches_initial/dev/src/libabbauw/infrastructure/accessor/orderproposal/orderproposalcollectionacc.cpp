    ...
    void SelectOrderProposalsByKey::buildSQLString()
    {
        ...
        static const basar::VarString sql( "\n"
			...
			+ "\n FROM orderproposal op \n"
              ...
              " \nLEFT JOIN cscbrancharticle af ON op.branchno = af.filialnr AND op.articleno = af.artikel_nr"
                                     " AND af.filialnr = "                                               + BRANCHNO.toSQLReplacementString()
            + " \nLEFT JOIN pubranchsupplier fl ON af.filialnr = fl.filialnr AND af.besla_nr = fl.besla_nr"
