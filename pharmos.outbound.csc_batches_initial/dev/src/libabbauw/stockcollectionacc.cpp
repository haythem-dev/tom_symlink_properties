...
namespace libabbauw
{
	namespace acc_stock
	{
		...
		void SelectPredictionDate::buildSQLString()
		{
			static const VarString sqlStatement("\n"
				"SELECT MAX (datum) "
				"\nFROM cscarticleprognosis "
				"\nWHERE filialnr = " + BRANCHNO.toSQLReplacementString() +
				"\n AND artikel_nr = "
				"(SELECT * FROM (SELECT FIRST 1 artikel_nr FROM cscbrancharticle WHERE filialnr = " + BRANCHNO.toSQLReplacementString() +
				"AND artikelaktiv = 1) )"
			);
		...
		void SelectStock::buildSQLString()
		{
			...
			static const VarString sqlStatement("\n"
				...
				" LEFT JOIN cscbrancharticle af ON (op.articleno = af.artikel_nr AND op.branchno = af.filialnr)\n"
				" LEFT JOIN cscarticleprognosis ap ON (op.branchno = ap.filialnr AND op.articleno = ap.artikel_nr"
