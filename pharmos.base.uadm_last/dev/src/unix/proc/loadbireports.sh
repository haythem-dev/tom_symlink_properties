...
#Purpose:           Starts, stops and status of script load bi reports (BSA003, BSC001)           
...
#LIBRARY PATH 
readonly BINARY=loadbireports.bin
...
readonly REPORT_OPTION="-report";
readonly FOLDER_OPTION="-folder";
...
function eval_parameters
{
	 ...
	 if [ ! "${report}" ]; then
		 printStartStop "${STARTMSG}"
		 rc=13;
		 echo "${PROCNAME}: No report set in command line!" | tee -a ${LOG_RESOURCE}
		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
		 exit ${rc};
	 fi
...
###############################################################################################
. $(dirname $0)/loadbireports.option
