...
namespace tender
{
    namespace viewConn
    {
        ...
        /*!    \throw  no-throw */
        /////////////////////////////////////////////////////////////////////////////////////////////
        TenderContractInfoFrance::TenderContractInfoFrance()
        : m_EventsWiring  ( "TenderContractInfoFrance::EventsWiring"   ),
		  m_Logger( libtender::LoggerPool::getLoggerViewConn() ),
		  m_StartSearchAction( NULL )
        {
            //METHODNAME_DEF( TenderContractInfoFrance, TenderContractInfoFrance )
        ...
        void TenderContractInfoFrance::wireEvents()
        {
			...
			connect( ui.lePartnerNo,		 SIGNAL( returnPressed() ), 
			this,							 SLOT  ( onCheckPartner() ) );

			connect( ui.lePartnerNo,		 SIGNAL( editingFinished() ), 
			this,							 SLOT  ( onCheckPartner() ) );
			...
			connect( ui.lePartnerNo,		 SIGNAL( textChanged(const QString &)  ),
			this,							 SLOT  ( onContractInfoChanged() ) );
		...
		void TenderContractInfoFrance::onStartSearch()
		{
			METHODNAME_DEF( TenderContractInfoFrance, onStartSearch )
			BLOG_TRACE_METHOD( m_Logger, fun );
		...
		void TenderContractInfoFrance::onCheckCustomer()
		{
			METHODNAME_DEF( TenderContractInfoFrance, OnCheckCustomer )
			BLOG_TRACE_METHOD( m_Logger, fun );

			basar::appl::SystemEventManager::getInstance().fire( "CheckCustomerRequested" );
		}

		void TenderContractInfoFrance::onCheckPartner()
		{
			METHODNAME_DEF( TenderContractInfoFrance, OnCheckPartner )
			BLOG_TRACE_METHOD( m_Logger, fun );

			basar::appl::SystemEventManager::getInstance().fire( "CheckPartnerRequested" );
		}

		const libtender::domMod::tender::TenderTypeCollectionPtr TenderContractInfoFrance::getTypeMapping()
		{
			return m_TenderTypeCollection;
		}

		const libtender::domMod::tender::TenderStateEnum TenderContractInfoFrance::getStateIndex()
		{
		...
		const libtender::domMod::tender::TenderStateEnum TenderContractInfoFrance::getStateData()
		{
		...
		const libtender::domMod::tender::TenderTypeEnum TenderContractInfoFrance::getType()
		{
		...
		const basar::Date TenderContractInfoFrance::getDateTo()
		{
		...
		bool TenderContractInfoFrance::isFilled()
		{
			METHODNAME_DEF( TenderContractInfoFrance, isFilled )
			BLOG_TRACE_METHOD( m_Logger, fun );

			if( ui.leContractNo->text().isEmpty() ) return false;
			if( ui.lePartnerNo->text().isEmpty()  ) return false;
			if( ui.leCustomerNo->text().isEmpty() ) return false;

			return true;
		}

		bool TenderContractInfoFrance::isCustomerSelected()
		{
			METHODNAME_DEF( TenderContractInfoFrance, isCustomerSelected )
			BLOG_TRACE_METHOD( m_Logger, fun );
		...
		bool TenderContractInfoFrance::isPartnerInFocus()
		{
			if( ui.lePartnerNo->hasFocus() ||
				ui.btnPartnerNo->hasFocus()  )
			{
				return true;
			}
		...
		bool TenderContractInfoFrance::isCustomerInFocus()
		{
		...
		bool TenderContractInfoFrance::isContractPeriodValid()
		{
		...
		void TenderContractInfoFrance::onContractPeriodClicked( bool isChecked )
		{
			if( isChecked )
			{
		...
		void TenderContractInfoFrance::onContractDateChanged()
		{
		...
		void TenderContractInfoFrance::onStateChanged( int index )
		{
			METHODNAME_DEF( TenderContractInfoFrance, onStateChanged )
			BLOG_TRACE_METHOD( m_Logger, fun );
		...
		void TenderContractInfoFrance::onContractInfoChanged()
		{
			METHODNAME_DEF( TenderContractInfoFrance, onTextChanged )
			BLOG_TRACE_METHOD( m_Logger, fun );

			basar::appl::SystemEventManager::getInstance().fire( "ContractInfoChanged" );
		}

        void TenderContractInfoFrance::matchToTender( basar::db::aspect::AccessorPropertyTable_YIterator yitTender )
        {
            ...
            {
                yitTender.setInt32 (properties::PARTNER_NO , ui.lePartnerNo->text().toInt());
            }
            ...
            if(!ui.leCustomerNo->text().isEmpty())
            {
				...
				yitTender.setInt32( properties::PERSONAL_NR_VERTR, ui.leSalesman->text().toInt() );
            }
		...
		void TenderContractInfoFrance::matchFromTender( basar::db::aspect::AccessorPropertyTable_YIterator yitTender, bool hasTenderArticles )
		{
			...
			if( yitTender.isContainedAndSet( properties::PARTNER_NO  ) )
			{
				ui.lePartnerNo->setText( QString::number( yitTender.getInt32( properties::PARTNER_NO ) ) );
			}
			...
			{
				ui.leCustomerName->setText( yitTender.getString( properties::PHARMACY_NAME ).c_str() );
			}

			if( yitTender.isContainedAndSet( properties::PERSONAL_NR_VERTR  ) )
			{
				ui.leSalesman->setText( QString::number( yitTender.getInt32( properties::PERSONAL_NR_VERTR ) ) );
			}

			if( yitTender.isContainedAndSet( properties::BRANCH_NO  ) )
			{
		...
		void TenderContractInfoFrance::matchFromCustomer( basar::db::aspect::AccessorPropertyTable_YIterator yitCustomer )
		{
			METHODNAME_DEF( TenderContractInfoFrance, matchFromCustomer )
			BLOG_TRACE_METHOD( m_Logger, fun );
			...
			else
			{
				...
				ui.leSalesman->setText( QString::number( yitCustomer.getInt32( properties::PERSONAL_NR_VERTR ) ) );

				if( 0 == yitCustomer.getInt16(  properties::ORIGINAL_BRANCH_NO ) )
				{
					ui.leCustomerBranchNo->setText( QString::number( yitCustomer.getInt16( properties::BRANCH_NO ) ) );
				}
				else
				{
					ui.leCustomerBranchNo->setText( QString::number( yitCustomer.getInt16( properties::ORIGINAL_BRANCH_NO ) ) );
				}
			}
		}

		void TenderContractInfoFrance::matchFromPartner( basar::db::aspect::AccessorPropertyTable_YIterator yitPartner )
		{
			METHODNAME_DEF( TenderContractInfoFrance, matchFromPartner )
			BLOG_TRACE_METHOD( m_Logger, fun );

			if (yitPartner.isNull() || yitPartner.isEnd())
			{
				ui.lePartnerNo->clear();
			}
			else
			{
				ui.lePartnerNo->setText( QString::number( yitPartner.getInt32( properties::PARTNER_NO ) ) );
			}
		}

		void TenderContractInfoFrance::matchToCustomerSearch( basar::db::aspect::AccessorPropertyTable_YIterator customerSearchYit )
		{
			METHODNAME_DEF( TenderContractInfoFrance, matchToCustomerSearch )
			BLOG_TRACE_METHOD( m_Logger, fun );
		...
		void TenderContractInfoFrance::matchToPartnerSearch( basar::db::aspect::AccessorPropertyTable_YIterator partnerSearchYit )
		{
			METHODNAME_DEF( TenderContractInfoFrance, matchToPartnerSearch )
			BLOG_TRACE_METHOD( m_Logger, fun );

			basar::Int32 partnerNo = ui.lePartnerNo->text().toInt();
			if( 0 < partnerNo )
			{
				partnerSearchYit.setInt32( properties::PARTNER_NO, partnerNo );
			}
			
			partnerSearchYit.setInt32( properties::TENDER_TYPE, ui.cbType->currentData().toInt() );
		}

        void TenderContractInfoFrance::setupContractInfo()
        {
            METHODNAME_DEF( TenderContractInfoFrance, setupContractInfo )
        ...
        void TenderContractInfoFrance::focusInEvent( QFocusEvent* event)
        {
            event;
            ui.leTenderNo->setFocus();
        }

        void TenderContractInfoFrance::focusOutEvent( QFocusEvent* event)
        {
            event;
        }

        void TenderContractInfoFrance::resetAllFields()
        {
            ui.leTenderNo->clear();
		...
		void TenderContractInfoFrance::resetCustomer()
		{
			ui.leCustomerName->clear();
			ui.leSalesman->clear();
			ui.leCustomerBranchNo->clear();
		...
		void TenderContractInfoFrance::setFocusOnFirstEmptyField()
		{
			if( ui.leContractNo->text().isEmpty() )
			{
        ...
        void TenderContractInfoFrance::injectStatusMapping( libtender::domMod::tender::TenderStateCollectionPtr statusMapping )
        {
        ...
        void TenderContractInfoFrance::injectTypeMapping( libtender::domMod::tender::TenderTypeCollectionPtr typeMapping )
        {
		...
		void TenderContractInfoFrance::setCustomerNo( basar::db::aspect::AccessorPropertyTable_YIterator yit )
		{
			METHODNAME_DEF( TenderContractInfoFrance, setCustomerNo )
            BLOG_TRACE_METHOD( libtender::LoggerPool::getLoggerViewConn(), fun );
		...
		void TenderContractInfoFrance::setPartnerNo( basar::db::aspect::AccessorPropertyTable_YIterator yit )
		{
            METHODNAME_DEF( TenderContractInfoFrance, setPartnerNo )
            BLOG_TRACE_METHOD( libtender::LoggerPool::getLoggerViewConn(), fun );
		...
		void TenderContractInfoFrance::disableAll()
		{
			...
			ui.lePartnerNo->setDisabled( true );
			ui.btnPartnerNo->setDisabled( true );
		...
		void TenderContractInfoFrance::disableAllExceptState( bool disable, bool hasTenderArticles )
		{
			if( disable )
			{
				ui.leContractNo->setDisabled( true );
			...
			else
			{
				...
				//Type is only enabled and changeable, when tender has no articles!
				...
				//Partner field is only enabled, when state is uncompleted!
				int index = ui.cbStatus->currentIndex();
				if( libtender::domMod::tender::TS_UNCOMPLETED == index )
				{
					ui.lePartnerNo->setEnabled( true );
					ui.btnPartnerNo->setEnabled( true );
				}
				else
				{
					ui.lePartnerNo->setDisabled( true );
