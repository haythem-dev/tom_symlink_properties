#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary csc_batches_orderproposal for presentation only.
 ###############################################################################################
 ...
 readonly BINARY=csc_batches_orderproposal.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${server}" == "" ]] || (( 1 != `check_string_parameter "${SERVER_OPTION}" "${server}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${server}: >${server}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $COUNTRYCODE_OPTION $countrycode $DB_OPTION $db $SERVER_OPTION $server"
 	 else
 		 _parameters="$COUNTRYCODE_OPTION $countrycode $DB_OPTION $db $SERVER_OPTION $server";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/csc_batches_orderproposal.option
